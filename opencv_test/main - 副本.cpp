		...
		for (size_t i = 0; i < s; i++)
		{
			...
			if (area > maxArea)
			{
				...
				//	imwrite(save_file, ROI_Head);
					...
					{
						rectangle(image, mr, Scalar(0, 0, 255), 2, 8, 0);
						putText(image, "Dengoues", Point(mr.x, mr.y), cv::FONT_HERSHEY_COMPLEX, 1, cv::Scalar(0, 0, 255), 1, 8, 0);
...
int BPXublian(void)
{
	...
	//(bp->setTrainMethod).setTermCriteria(CV_TERMCRIT_ITER | CV_TERMCRIT_EPS, 10000, 0.0001);
	//	B* p = &b;
	//p->fun();
